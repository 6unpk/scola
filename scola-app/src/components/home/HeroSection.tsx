...
export default function HeroSection() {
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q) return SUGGESTION_POOL.slice(0, 6);
  ...
  return (
    <Hero>
      ...
      <HeroInner>
        <HeroLogo>
          <Image src={logoSrc} alt="Scola" width={320} height={100} priority style={{ filter: 'brightness(0) invert(1)', opacity: 0.95 }} />
        </HeroLogo>
        ...
        <HeroSearchWrap ref={wrapRef}>
          ...
          {showSuggestions && (
            <SuggestionsBox>
              ...
              {suggestions.map((s) => (
                <SuggestionItem key={s} type="button" onClick={() => handleSelectSuggestion(s)}>
                  {query.trim()
